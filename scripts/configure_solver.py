...
def create_solver_config(solver_name=None):
    ...
    config = {
        'smt': {
            ...
            'z3_bin': 'z3'  # Explicitly set z3 binary path
        }
    }
    ...
    if solver_choice != 'auto':
        ...
        if required_binary not in available:
            print(f"ERROR: Solver '{solver_choice}' not found (looking for binary '{required_binary}')")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--solver', choices=['z3', 'yices', 'cvc4', 'boolector', 'auto'],
                       help='SMT solver to use (default: auto or $MANTICORE_SOLVER)')
    parser.add_argument('--check', action='store_true',
                       help='Check available solvers')
    parser.add_argument('--symlinks', action='store_true',
                       help='Create solver symlinks if needed')
