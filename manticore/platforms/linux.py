...
class Linux(Platform):
    ...
    def sys_writev(self, fd, iov, count):
        ...
        for i in range(0, count):
            ...
            if issymbolic(size):
                size = YicesSolver.instance().get_value(self.constraints, size)
...
class SLinux(Linux):
    ...
    def _transform_write_data(self, data: MixedSymbolicBuffer) -> bytes:
        ...
        for c in data:
            if issymbolic(c):
                bytes_concretized += 1
                c = bytes([YicesSolver.instance().get_value(self.constraints, c)])
    ...
    def sys_exit_group(self, error_code):
        if issymbolic(error_code):
            error_code = YicesSolver.instance().get_value(self.constraints, error_code)
            return self._exit(
    ...
    def generate_workspace_files(self) -> Dict[str, Any]:
        def solve_to_fd(data, fd):
            ...
            try:
                for c in data:
                    if issymbolic(c):
                        c = YicesSolver.instance().get_value(self.constraints, c)
                    fd.write(make_chr(c))
            except SolverError:
