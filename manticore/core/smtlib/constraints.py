...
#            result += '(assert %s)\n' % translate_to_smtlib(prop, bindings=True)
    ...
    def new_array(self, index_bits=32, value_bits=8, name='A', index_max=None, taint=frozenset()):
        ...
        name = self._get_new_name(name)
        return ArrayProxy(ArrayVariable(index_bits, value_bits, index_max, name, taint=taint))
