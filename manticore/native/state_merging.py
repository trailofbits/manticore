from ..core.smtlib import YicesSolver, ConstraintSet, Operators, issymbolic, BitVec
...
def compare_sockets(cs, socket1, socket2):
    ...
    :param cs: ConstraintSet to be used for checking Socket.buffer for semantic equality using `YicesSolver.instance().must_be_true()`
...
def compare_buffers(cs, buffer1, buffer2):
    """
    This method compares the two List objects for equality using the `YicesSolver.instance().must_be_true()` call.
    :param cs: ConstraintSet to be used for checking buffer1 for semantic equality with buffer2 using `YicesSolver.instance().must_be_true()`
    ...
    for b1, b2 in zip(buffer1, buffer2):
        if not YicesSolver.instance().must_be_true(cs, b1 == b2):
            return False
    return True
...
def compare_byte_vals(mem1, mem2, addr, merged_constraint):
    ...
    :param merged_constraint: ConstraintSet to be used when using the call to `YicesSolver.instance().must_be_true()`
    ...
    if not YicesSolver.instance().must_be_true(merged_constraint, cond_to_check):
        return False
    else:
        return True
...
def compare_mem(mem1, mem2, merged_constraint):
    ...
    :param merged_constraint: ConstraintSet object that is to be used with `YicesSolver.instance().must_be_true()` calls to check the
...
def merge_cpu(cpu1, cpu2, state, exp1, merged_constraint):
    ...
    satisfiable as checked using `YicesSolver.instance().must_be_true()`
    ...
    for reg in cpu1.canonical_registers:
        ...
        if issymbolic(val1) or issymbolic(val2) or val1 != val2:
            if YicesSolver.instance().must_be_true(merged_constraint, val1 != val2):
